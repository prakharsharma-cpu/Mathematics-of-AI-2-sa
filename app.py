...
if uploaded_file is not None:
    try:
        ...
        st.write("Columns detected in your CSV:", df.columns.tolist())

        # Check required columns
        required_columns = ['player_name', 'club', 'season', 'match_date', 
                            'injury_type', 'performance_before', 'performance_after', 'age']
        missing_cols = [col for col in required_columns if col not in df.columns]

        if missing_cols:
            st.error(f"Uploaded CSV is missing these required columns: {missing_cols}")
        else:
            # Parse match_date safely
            df['match_date'] = pd.to_datetime(df['match_date'], dayfirst=True, errors='coerce')
            
            # Drop rows where match_date failed to parse
            ...
            df.dropna(subset=['performance_before', 'performance_after', 'age'], inplace=True)
